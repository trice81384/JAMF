...
## if so delete it.
## Scripts can be found by id # at the following endpoint:
## /JSSResource/scripts/id/#
...
## ARGV $1 is the name of the Script we want to delete while
...
## If a positive match is found then it means the Script we want to delete exists.
## So using curl we can DELETE the Script from the instance.
## We have to delete the Script by its ID number which is why we retrieved it in the previous loop.
...
printf "\n"
echo "Script:$ScriptName Deleted for $CompanyName"
echo ""
else
