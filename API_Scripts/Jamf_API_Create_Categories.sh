...
## This script is designed to check if a Category exists on a Jamf Pro Server and
## if not to upload a new one.
## Categories can be uploaded by id to the following endpoint:
## /JSSResource/categories/id/0

##Any references to the JSS are due to historical reasons but JSS = Jamf Pro Server

### When using the XML downloaded from the JSS we must strip the ID tags from the category 
## and any categories associated with the category. We also must make sure any associated
...
## ARGV $1 is the path to the xml file of the Category we want to upload/create
...
## Search for the actual (Display) name of the category we are going to create by reading in the XML file for the category.
## We first grep for <category> which is the opening tag or first line of the file and return that and the 1 line below it
## which (2nd line) contains the name of the category. Then working on those 2 lines returned we use awk with <name> or </name>
## as the field separators which will return only the data between those tags which corresponds to the category's name. Since awk 
...
## the returned data which is guaranteed to be the entire category name.
## In case there's any white space we use sub and a regular expression (^ ) to replace it with nothing ""

CategoryName=$(cat "$CategoryXML" | grep -A 1 "<category>" | awk -F "<name>|</name>" 'NR==2{sub(/^ /,"");print $2}')
...
## a list of all the names of the Categories in the JSS.
## We'll first match the top element of the list of Categories returned from the JSS
## which is categories. Then from each child element (category)
...
<xsl:template match="/categories"> <!-- Start a search -->
...
## output the full list of categories from the JSS to a file.
## The list of Categories can be found at the /categories endpoint 
...
## Since outputting the list of Categories to a file just dumps the raw XML to the file
...
xsltproc /tmp/JSS_template.xslt /tmp/JSS_output.xml > /tmp/JSS_categorylist.txt

## Read in the previous text file line by line to see if it contains the name of the Category
...
done < /tmp/JSS_categorylist.txt

## If a positive match is never found then it means the category we want to create does not already exist
## so using curl we can POST a new category to the JSS instance. We send it to the 0 endpoint since this allows
...
printf "\n"
echo "Category:$CategoryName Created for $CompanyName"
echo ""
else
echo "Category *$CategoryName* already exists for $CompanyName" 
echo ""
fi
