...
## This script is designed to check if a Building exists on a Jamf Pro Server and
## if not to upload a new one.
## Buildings can be uploaded by id to the following endpoint:
## /JSSResource/buildings/id/0
...
## ARGV $1 is the name of the Building (either singular or from a text file) we want to upload/create
...
## a list of all the names of the Buildings in the JSS.
## We'll first match the top element of the list of Buildings returned from the JSS
## which is buildings. Then from each child element (building)
...
<xsl:template match="/buildings"> <!-- Start a search -->
...
## output the full list of buildings from the JSS to a file.
## The list of Buildings can be found at the /buildings endpoint
...
## Since outputting the list of buildings to a file just dumps the raw XML to the file
...
## and then output the formatted XML (which in this case is just the names of all the buildings)
## to a text file.

xsltproc /tmp/JSS_template.xslt /tmp/JSS_output.xml > /tmp/JSS_buildinglist.txt 

## Read in the new buildings from the variable populated at the begging of the script.
## Either passed on the command line or defined in the script.
## Buildings can be listed in a text file line by line. Make sure the file has a blank line at the end
## and NO Windows line breaks.
## Then do a grep to compare each new building read in to the buildings in the JSS_buildinglist we created earlier.
## We use grep with the -e option to check for a supplied pattern (NewBuildingName) against the supplied input (JSS_buildinglist)
## The -q option runs grep in quiet mode to suppress extraneous output and the -i option so that pattern checking is not case sensitive.
## If grep does not find a match in the existing buildings with the new building its output code is 1 which we check with the $? variable
## If the output code is 1 we expect the building not to already exist and create it with the API call.
## We let the user know if a new building was created or found. 

while read NewBuildingName
do
grep -qi -e "$NewBuildingName" /tmp/JSS_buildinglist.txt
if [ $? == 1 ]
then
curl \
-s \
...
$JSSResource/buildings/id/0 \
-d "<building><name>$NewBuildingName</name></building>" \
-X POST
printf "\n"
echo "Building:$NewBuildingName Created for $CompanyName"
echo ""
else
echo "Building: *$NewBuildingName* already exists for $CompanyName" 
echo ""
fi
done < "$BuildingNames"
