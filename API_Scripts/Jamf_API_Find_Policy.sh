...
## This script is designed to check if a Policy exists on a Jamf Pro Server.
...
## ARGV $1 is the name of the Policy we want to find while
...
## we want to find. We check for a positive match one time only to avoid continual iterations of the
...
## variable (the contents of which we want to use later). If the contents of PolicySearch match the name of the 
## policy we want to fine in PolicyName then we set the PolicyFound variable to yes and pull in the ID number for
## that policy from the Policy list we created earlier.
## Since the Policy list we created earlier has the policy name and id separated by a ? we use the awk statement with a ?
## as a field separator to find either the Policy name (first field) or id (second field). We only search for the id if 
## we find the name of the policy we are looking for.
...
then
PolicyFound="Yes"
PolicyFoundName="$PolicySearch"
Policyid=$(echo "$name" | awk -F "\?" '{print $2}')
fi
done < /tmp/JSS_PolicyList.txt


## If a positive match is found then it means the policy we are looking for exists.
## Print out the Policy Name, the company, and its JAMF URL.

if [ "$PolicyFound" == "Yes" ]
then
echo "FOUND***Policy: $PolicyFoundName found for $CompanyName***$instanceName"
#printf "\n"
else
echo "NOT FOUND***Policy: $PolicyName not found for $CompanyName***$instanceName" 
#printf "\n"
fi
