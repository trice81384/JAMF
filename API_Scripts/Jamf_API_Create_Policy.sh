...
## Policies can be uploaded by id to the following endpoint:
...
## ARGV $1 is the path to the xml file of the Policy we want to upload/create
...
## Search fir the actual (Display) name of the policy we are going to create by reading in the XML file for the policy.
## We first grep for <policy> which is the opening tag or first line of the file and return that and the 2 lines below it
## the last of which (3rd line) contains the name of the policy. Then working on those 3 lines returned we use awk with <name> or </name> 
## as the field separators which will return only the data between those tags which corresponds to the policy's name. Since awk
## will not find any data on the first 2 lines we use NR==3 to tell awk to only return the 3rd line and just the second field from
## the returned data which is guaranteed to be the entire policy name.
## In case there's any white space we use sub and a regular expression (^ ) to replace it with nothing ""

PolicyName=$(cat "$PolicyXML" | grep -A 2 "<policy>" | awk -F "<name>|</name>" 'NR==3{sub(/^ /,"");print $2}')
...
## and then output the formatted XML (which in this case is just the names of all the policies)
## to a text file.
...
## If a positive match is never found then it means the policy we want to create does not already exist
## so using curl we can POST a new policy to the JSS instance. We send it to the 0 endpoint since this allows
...
printf "\n"
echo "Policy:$PolicyName Created for $CompanyName"
echo ""
else
echo "Policy: *$PolicyName* already exists for $CompanyName" 
echo ""
fi
